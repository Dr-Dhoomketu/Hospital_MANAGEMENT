...
export default function PortalDashboardPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F7FC' }}>
      {/* Topbar */}
      <header style={{
        background: '#0A1F44', padding: '0 32px', height: '64px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button onClick={() => signOut({ redirectUrl: '/' })} style={{
            ...
            background: 'transparent', color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer',
          }}>Sign Out</button>
        </div>
      </header>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: '#0A1F44' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '32px' }}>
          {[
            { title: 'Next Appointment', value: 'Sep 23, 10:00 AM', sub: 'Dr. Arjun Mehta · Cardiology', color: '#2F5DAA' },
            { title: 'Active Prescriptions', value: '3', sub: 'View in pharmacy', color: '#16a34a' },
            { title: 'Lab Results', value: '1 New', sub: 'Lipid profile ready', color: '#d97706' },
            { title: 'Unpaid Bills', value: '₹2,400', sub: 'Invoice #INV-0031', color: '#dc2626' },
          ].map(card => (
            <div key={card.title} style={{
              background: '#fff', borderRadius: '20px', padding: '24px',
              border: '1px solid rgba(10,31,68,0.08)', boxShadow: '0 2px 12px rgba(10,31,68,0.04)',
            }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <Link href="/book">
            <div style={{
              background: '#0A1F44', borderRadius: '20px', padding: '28px', cursor: 'pointer',
